...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, RouterLink, MoneyPipe, StatusChipComponent, LoadingComponent, ErrorStateComponent],
  ...
  template: `
    @if (store.loading()) {
      ...
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div>
          <a routerLink="/app/quotations" class="text-sm text-slate-500 hover:text-slate-800">← All quotations</a>
      ...
      <div class="mt-6 grid gap-6 xl:grid-cols-3">
        ...
        <section class="xl:col-span-2">
            ...
            <table class="min-w-full divide-y divide-slate-200">
              ...
              <tbody class="divide-y divide-slate-100">
                @for (line of store.pricedLines(); track line.id) {
                  <tr [class]="line.discountStatus === 'OVER' ? 'bg-rose-50' : ''">
                    <td class="df-td">
                      ...
                      <p class="text-xs text-slate-400">
                        ...
                        @if (line.addedFromUpsell) { <span class="ml-1 rounded bg-brand-50 px-1.5 py-0.5 text-[10px] font-medium text-brand-700">from upsell</span> }
                      </p>
                    </td>
                    ...
                    <td class="df-td">
                      @if (line.discountStatus === 'OVER') {
                        <span class="df-chip border-rose-200 bg-rose-100 text-rose-800">OVER (+{{ line.overByPts }}pt)</span>
                      } @else {
                        <span class="df-chip border-emerald-200 bg-emerald-100 text-emerald-800">OK</span>
                      }
                    </td>
                  ...
                  <tr><td class="df-td py-10 text-center text-slate-400" colspan="8">No lines yet. Add a product to get started.</td></tr>
                }
              </tbody>
            </table>
          </div>
          ...
          <p class="mt-2 text-xs leading-relaxed text-slate-500">
            Each line's discount is checked against <em>its own</em> limit — the stricter of the customer's tier ceiling and the
            product category's ceiling — as soon as you enter it, not only when you submit.
          </p>
        ...
        <aside class="space-y-4">
          ...
          <div class="df-card p-5">
            ...
            <dl class="mt-3 space-y-1.5 text-sm">
              <div class="flex justify-between"><dt class="text-slate-500">Subtotal</dt><dd class="font-medium">{{ store.subtotal() | money }}</dd></div>
              <div class="flex justify-between"><dt class="text-slate-500">Discount</dt><dd class="font-medium text-rose-600">−{{ store.discountTotal() | money }}</dd></div>
              <div class="flex justify-between"><dt class="text-slate-500">Tax</dt><dd class="font-medium">{{ store.taxTotal() | money }}</dd></div>
              <div class="mt-2 flex justify-between border-t border-slate-200 pt-2 text-base">
                <dt class="font-semibold text-slate-700">Total</dt><dd class="font-semibold text-slate-900">{{ store.grandTotal() | money }}</dd>
              </div>
              @if (store.recurringTotal() > 0) {
                <div class="flex justify-between pt-1 text-xs">
                  <dt class="text-slate-400">of which recurring, per cycle</dt><dd class="text-slate-500">{{ store.recurringTotal() | money }}</dd>
                </div>
              }
            </dl>
            <div class="mt-4 rounded-lg p-3" [class]="store.marginPct() >= 20 ? 'bg-emerald-50' : store.marginPct() >= 10 ? 'bg-amber-50' : 'bg-rose-50'">
              <p class="text-xs font-medium uppercase tracking-wide"
                 [class]="store.marginPct() >= 20 ? 'text-emerald-700' : store.marginPct() >= 10 ? 'text-amber-700' : 'text-rose-700'">Margin</p>
              <p class="mt-0.5 text-2xl font-semibold"
                 [class]="store.marginPct() >= 20 ? 'text-emerald-800' : store.marginPct() >= 10 ? 'text-amber-800' : 'text-rose-800'">
          ...
          <div class="df-card p-5">
            <div class="flex items-center justify-between">
              ...
              <df-status-chip kind="risk" [value]="store.risk().riskLevel" [text]="store.risk().riskLevel + ' · ' + store.risk().riskScore" />
            </div>
                  ...
                  <li class="rounded-lg border border-slate-200 p-3">
                    <div class="flex items-start justify-between gap-2">
                      <div class="min-w-0">
                        ...
                        <p class="text-xs text-emerald-600">Margin +{{ s.marginDelta | money }}</p>
...
export class QuotationDetailPage implements OnInit {
  ...
  ngOnInit(): void { void this.store.load(this.id()); }
  reload(): void { void this.store.load(this.id()); }
