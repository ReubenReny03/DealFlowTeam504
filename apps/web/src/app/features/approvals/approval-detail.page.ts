...
import { CATEGORY_LABEL, ROLE_LABEL } from '@dealflow/shared';
import { ApprovalStore } from '../../core/state/feature.stores';
import { ToastStore } from '../../core/state/toast.store';
import {
  ConfirmDialogComponent, ErrorStateComponent, LoadingComponent, LongDatePipe,
  MoneyPipe, Step, StepperComponent, StatusChipComponent,
} from '../../shared/ui';
...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, StepperComponent, StatusChipComponent, ConfirmDialogComponent, MoneyPipe, LongDatePipe, LoadingComponent, ErrorStateComponent],
  ...
  template: `
    @if (store.loading()) {
      ...
      <a routerLink="/app/approvals" class="text-sm text-slate-500 hover:text-slate-800">← Approvals</a>
      ...
      <div class="mt-1 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div class="mt-2 flex flex-wrap items-center gap-2">
            <df-status-chip kind="risk" [value]="a.risk.riskLevel" [text]="'Blended Risk: ' + a.risk.riskLevel" />
            <df-status-chip kind="tier" [value]="a.tier" [text]="'Customer Tier: ' + a.tier" />
            <df-status-chip kind="approval" [value]="a.status" />
            <span class="df-muted">Submitted by {{ a.ownerName }} · {{ a.amount | money: a.currency }}</span>
          </div>
        </div>
        <a [routerLink]="['/app/quotations', a.quotationId]" class="df-btn-ghost">Open the quotation</a>
      </div>
      ...
      <div class="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <section class="lg:col-span-2">
                ...
                @for (row of a.risk.explanation; track row.lineId) {
                  <tr [class]="row.status === 'OVER' ? 'bg-rose-50' : ''">
                    ...
                    <td class="df-td text-right">
                      @if (row.status === 'OVER') {
                        <span class="font-mono font-semibold text-rose-700">{{ row.overBy }} pt OVER</span>
                      } @else {
                        <span class="font-mono text-emerald-700">0 pt OK</span>
                      }
                    </td>
                    <td class="df-td text-right font-mono text-slate-400">{{ (row.weight * 100).toFixed(0) }}%</td>
                  </tr>
                }
              </tbody>
            </table>
          ...
          <p class="mt-2 text-xs leading-relaxed text-slate-500">
            The worst single line over its limit, plus the revenue-weighted pattern across the whole order, together set the blended
            score. One bad line is enough to require approval — and many small ones are too.
          </p>
          ...
          <div class="df-card df-scroll-x mt-3">
            <table class="min-w-full divide-y divide-slate-200">
              <thead class="bg-slate-50">
                <tr>
                  <th class="df-th">User</th><th class="df-th">Action</th><th class="df-th">Date</th><th class="df-th">Reason</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-100">
                @for (entry of a.trail; track entry.at) {
                  ...
                  <tr><td class="df-td py-6 text-center text-slate-400" colspan="4">No actions recorded yet.</td></tr>
                }
              </tbody>
            </table>
          </div>
        ...
        <aside>
          <div class="df-card p-5">
            ...
            @if (canAct()) {
              <p class="df-muted mt-1">This quotation is waiting on {{ a.assignedToName ?? roleLabel(a.currentStage) }}.</p>
              <div class="mt-4 space-y-2">
                <button type="button" class="df-btn-success w-full" (click)="ask('approve')">Approve</button>
                <button type="button" class="df-btn-warn w-full" (click)="ask('return')">Return for Revision</button>
                <button type="button" class="df-btn-danger w-full" (click)="ask('reject')">Reject</button>
              </div>
          ...
          @if (a.reEnteredFromNegotiation) {
            <div class="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-4">
              ...
              <p class="mt-1 text-sm leading-relaxed text-sky-800">
                The customer changed the terms in the portal and the quotation re-entered approval by itself. Nobody asked for this review.
              </p>
            </div>
          }
        </aside>
...
export class ApprovalDetailPage implements OnInit {
  ...
  roleLabel(r?: string): string { return r ? (ROLE_LABEL[r as keyof typeof ROLE_LABEL] ?? r) : 'nobody'; }
  ...
  readonly steps = computed<Step[]>(() => {
    ...
    const chain: Step[] = a.steps.map((s) => ({
      ...
      state:
        ...
        : s.status === 'REJECTED' ? 'failed'
        : s.status === 'RETURNED' ? 'failed'
        : 'pending',
  ...
  ask(action: 'approve' | 'return' | 'reject'): void { this.pendingAction.set(action); }

  dialogTitle(): string {
    return { approve: 'Approve this quotation?', return: 'Return for revision?', reject: 'Reject this quotation?' }[this.pendingAction() ?? 'approve'];
  }
  dialogMessage(): string {
    const a = this.store.current();
    switch (this.pendingAction()) {
      case 'return': return `${a?.quotationNumber} goes back to ${a?.ownerName} as a draft. They will see your reason and can resubmit.`;
      case 'reject': return `${a?.quotationNumber} is closed for good. The rep would have to start a new quotation.`;
      default: return a?.steps.some((s) => s.status === 'PENDING')
  ...
  dialogConfirm(): string {
    return { approve: 'Approve', return: 'Return for Revision', reject: 'Reject' }[this.pendingAction() ?? 'approve'];
  }
  dialogTone(): 'success' | 'warn' | 'danger' {
    return { approve: 'success' as const, return: 'warn' as const, reject: 'danger' as const }[this.pendingAction() ?? 'approve'];
  }
