...
@Component({
  ...
  template: `
    ...
    <p class="df-muted mt-1">Nothing is billed before it ships — a partial delivery produces a partial invoice.</p>

    <div class="mt-4 flex flex-wrap gap-2">
      <span class="df-chip border-rose-200 bg-rose-100 text-rose-800">{{ counts().unpaid }} Unpaid</span>
      <span class="df-chip border-emerald-200 bg-emerald-100 text-emerald-800">{{ counts().paid }} Paid</span>
      @if (counts().overdue > 0) { <span class="df-chip border-rose-300 bg-rose-200 text-rose-900">{{ counts().overdue }} Overdue</span> }
    </div>
      ...
      <div class="mt-4">
        <df-data-table [columns]="columns" [rows]="items()" [clickable]="true" (rowClick)="open($event)">
          <ng-template #cell let-row let-col="col">
            @switch (col.key) {
              @case ('type') { <span class="text-slate-500">{{ row.type === 'RECURRING' ? 'Recurring' : 'One-time' }}</span> }
              @case ('amount') { <span class="font-semibold">{{ row.total | money: row.currency }}</span> }
              @case ('status') { <df-status-chip kind="invoice" [value]="row.status" /> }
              @case ('due') { <span>{{ row.dueDate | shortDate }}</span> }
...
export class InvoiceListPage implements OnInit {
  ...
  counts() { return this.store.invoices()?.counts ?? { unpaid: 0, paid: 0, overdue: 0 }; }
  items() { return this.store.invoices()?.items ?? []; }

  ngOnInit(): void { void this.store.loadInvoices(); }
  reload(): void { void this.store.loadInvoices(); }
  open(row: InvoiceDto): void { void this.router.navigate(['/app/invoices', row.id]); }
}
