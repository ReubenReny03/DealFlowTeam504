...
mountModuleHealth(reportingRouter, {
  ...
  implemented: [
    ...
    'GET /export.csv', 'GET /export.pdf', 'GET /export.xlsx',
  ],
  todo: [],
});
...
reportingRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="dealflow360-reporting-${Date.now()}.csv"`);
    res.send(csv);
  }),
);
  ...
  asyncHandler(async (req, res) => {
    ...
    const pdf = buildSimplePdf({
      ...
      summary: [
        ...
        ['Avg Approval Time', d.avgApprovalTimeLabel],
        ['Top Upsell Product', d.topUpsellProduct ? `${d.topUpsellProduct.name} (${d.topUpsellProduct.timesAdded}x)` : '—'],
        ['Total Quoted Value', formatMoney(d.totalQuotedValue)],
        ['Total Confirmed Value', formatMoney(d.totalConfirmedValue)],
        ['Conversion Rate', `${d.conversionRatePct}%`],
      ],
      sections: [
        ...
        },
        {
          ...
          rows: d.byProduct.slice(0, 15).map((p) => [p.name, p.qty, formatMoney(p.value), p.avgDiscountPct]),
        },
      ],
    });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="dealflow360-reporting-${Date.now()}.pdf"`);
    res.send(pdf);
  }),
);
...
reportingRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows: XlsxRow[] = [
      ...
      [{ value: 'Quotes Created', style: 'text' }, { value: d.quotesCreated, style: 'int' }],
      [{ value: 'Avg Approval Time', style: 'text' }, { value: d.avgApprovalTimeLabel, style: 'text' }],
      [
        { value: 'Top Upsell Product', style: 'text' },
        { value: d.topUpsellProduct?.name ?? '—', style: 'text' },
        { value: d.topUpsellProduct?.timesAdded ?? '', style: 'int' },
      ],
      [{ value: 'Total Quoted Value', style: 'text' }, { value: money(d.totalQuotedValue), style: 'currency' }],
      [{ value: 'Total Confirmed Value', style: 'text' }, { value: money(d.totalConfirmedValue), style: 'currency' }],
      [{ value: 'Conversion Rate', style: 'text' }, { value: d.conversionRatePct, style: 'percent' }],
      [],
      [{ value: 'By stage', style: 'label', colSpan: 4 }],
      [{ value: 'Stage', style: 'header' }, { value: 'Count', style: 'header' }, { value: 'Value', style: 'header' }],
      ...d.byStage.map(
        (s): XlsxRow => [
      ...
      [
        { value: 'Rep', style: 'header' }, { value: 'Quotes', style: 'header' },
        { value: 'Value', style: 'header' }, { value: 'Avg Discount %', style: 'header' },
      ],
      ...d.byRep.map(
        (r): XlsxRow => [
      ...
      [
        { value: 'Product', style: 'header' }, { value: 'Qty', style: 'header' },
        { value: 'Value', style: 'header' }, { value: 'Avg Discount %', style: 'header' },
      ],
      ...d.byProduct.map(
        (p): XlsxRow => [
    ...
    res.setHeader('Content-Disposition', `attachment; filename="dealflow360-reporting-${Date.now()}.xlsx"`);
    res.send(xlsx);
  }),
);
